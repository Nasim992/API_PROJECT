...
public class TokenController : ControllerBase
{
    ...
    // 1. The POST Endpoint (Reads from Postman's x-www-form-urlencoded body)
    [HttpPost("api/token")]
    public async Task<IActionResult> TokenPost([FromForm] TokenRequest request)
    {
        return await ProcessTokenGeneration(request);
    }

    [HttpGet("api/token")]
    public async Task<IActionResult> TokenGet([FromQuery] TokenRequest request)
    {
        return await ProcessTokenGeneration(request);
    }

    // 3. Your Existing Logic (No attributes needed here)
    private async Task<IActionResult> ProcessTokenGeneration(TokenRequest request)
    {
        try
        {
            var username = request.UserName?.Trim();
